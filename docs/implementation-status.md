...
| Area | Implemented |
| --- | --- |
| Control plane API | Dev auth, OIDC bearer validation with IdP group → `platform_admin` promotion (one-way, `SKQUAD_OIDC_ADMIN_GROUPS`), LAN/Tailscale-only break-glass admin login (argon2id, CIDR allowlist, rate-limited, fail-closed), squads, agents, boards, tasks, registry, permissions, access grants, audit/metering reads, gateway metering callback, lease-backed/fenced agent task APIs, retry/dead-letter-aware message APIs, agent chat history endpoint, agent work-wait endpoint, context APIs, Kubernetes outbox intents, and the task execution reaper (expires lapsed leases and re-queues stuck tasks; see [`execution-reaper.md`](execution-reaper.md)). |
